...
HI_S32 GPSMNG_ParseRawData(GPSMNG_RAW_DATA* gpsRawData, HI_GPSMNG_MSG_PACKET* gpsMsgPack)
{
    HI_S32 s32Ret = HI_SUCCESS;
	
    /* parser RMC rawdata 推荐最小定位信息*/
	...
	//MLOGI("\nlatitude value: %d latitude.scale: %d longitude.value: %d longitude.scale: %d\n",gpsMsgPack->gpsGLL.latitude.value,gpsMsgPack->gpsGLL.latitude.scale,gpsMsgPack->gpsGLL.longitude.value,gpsMsgPack->gpsGLL.longitude.scale);
#ifdef GPSMNG_DEBUG
    MLOGI("\nGLL Parser Result:\n time: %d/%d/%d/%d \n latitude: %d %d \n longitude : %d %d\n status: %c\n mode: %c\n",
